...
RunStat solveInstance_test(const Instance &inst) {
    ...
    // 计时器累计变量
    double t_genInitial = 0.0;
    double t_evalInitial = 0.0;
    double t_neighborGen = 0.0;
    double t_evalNeighbor = 0.0;
    double t_loopOverhead = 0.0;
    ...
    Solution best = genInitial(inst);
    auto t1 = Clock::now();
    t_genInitial = ms_t(t1 - t0).count();

    auto t2 = Clock::now();
    double bestObj = evaluate(inst, best);
    auto t3 = Clock::now();
    t_evalInitial = ms_t(t3 - t2).count();
    ...
    while (R < N_NON_IMPROVING) {
        ...
        for (int it = 0; it < iterPerTemp; ++it) {
            ...
            auto ng0 = Clock::now();
            Solution cand = (c <= 10)
                            ? neighbor_4operator(inst, cur)
                            : neighbor_6operator(inst, cur);
            auto ng1 = Clock::now();
            t_neighborGen += ms_t(ng1 - ng0).count();

            // 2) 邻域评估计时
            auto ne0 = Clock::now();
            double obj = evaluate(inst, cand);
            auto ne1 = Clock::now();
            t_evalNeighbor += ms_t(ne1 - ne0).count();

            // 3) Metropolis 接受准则及其它开销
            auto oh0 = Clock::now();
            double diff = obj - curObj;
            if (diff <= 0 || rand01() < std::exp(-diff / T)) {
                cur = cand;
                curObj = obj;
                if (curObj < bestObj) {
                    best = cur;
            ...
            auto oh1 = Clock::now();
            t_loopOverhead += ms_t(oh1 - oh0).count();
        }
    ...
    double totalMs = ms_t(outer_t1 - outer_t0).count();

    return {bestObj, totalMs};
