...
double evaluate_omp(const Instance &inst, const Solution &S) {
    int c = inst.c;
    double truckT = 0, droneT = 0;
    int pos = 0;
    // c客户数量少的时候不需要多线程，反而会增加开销
// # pragma omp parallel for reduction(+:truckT) reduction(max:droneT) private(pos)
    for (int k = 0; k < c; ++k) {
        int cust = S.seq[k];
        truckT += inst.tauTruck[pos][cust];
        pos = cust;
...
RunStat solveInstance(const Instance &inst) {
    ...
    while (R < N_NON_IMPROVING) {
        ...
        for (int it = 0; it < iterPerTemp; ++it) {
            ...
            double obj = evaluate_omp(inst, cand);
