...
struct DeviceInstance {
    int c;
    double SL, SR, droneEndurance;
    double *tauTruck;   // (c+2)*(c+2) 打平行优先
    double *tauDrone;   // 同上
};

// ------- 打平工具函数 (Host 侧) ------- //
inline void flattenInstance(const Instance &src, DeviceInstance &dst, std::vector<double> &host_tauTruck,
                            std::vector<double> &host_tauDrone) {
    dst.c = src.c;
    dst.SL = src.SL;
    dst.SR = src.SR;
    dst.droneEndurance = src.droneEndurance;
    int n = src.c + 2;
    host_tauTruck.resize(n * n);
    host_tauDrone.resize(n * n);
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            host_tauTruck[i * n + j] = src.tauTruck[i][j];
            host_tauDrone[i * n + j] = src.tauDrone[i][j];
        }
    }
}
